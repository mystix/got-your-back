  ...
  if exception is not None:
    try:
      ...
      if error['error']['code'] == 400:
        print("\n[message_num: %s] ERROR: %s: %s. Skipping message restore, you can retry later with --fast-restore"
          % (request_id, error['error']['code'], error['error']['errors'][0]['message']))
        return
    except:
      pass
    raise exception
...
def main(argv):
 ...
 # RESTORE-MBOX #
    ...
    sqlcur.executescript('''CREATE TABLE
                        IF NOT EXISTS mbox_resume.mbox_messages
                        (message_num TEXT PRIMARY KEY,
                         message_filename TEXT,
                         rfc822_msgid TEXT)''')
